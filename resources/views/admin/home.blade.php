            ...
            <div class="panel panel-default">
				...
				<div class="tab-content">
					...
					<div role="tabpanel" class="tab-pane fade in active" id="ostudents">
						...
						<div class="panel-body">
							...
							@foreach ( $one2ones as $student )
								...
								@endif
									<td>{{ $student -> ename }}</td>
									<td><a href="{{ url('lessonsinfo') . '/' . $student -> id }}">{{ $student -> name }}</a> &emsp;<span class="{{ $student -> getfuxi() > 5 ? 'color-danger' : ( $student -> getfuxi() == 5 ? 'color-warning' : 'hidden' ) }}">复</span>&emsp;<span class="{{ $student -> getjingpin() > 5 ? 'color-danger' : ( $student -> getjingpin() == 5 ? 'color-warning' : 'hidden' ) }}">精</span></td>
									<td>{{ $student -> sex }}</td>
							...
							<table class="table table-hover">
								<tr><th>班级ID</th><th>班级名称</th><th>上课地点</th><th>上课内容</th><th>班级人数</th>{{--<th>操作</th>--}}</tr>
									@foreach ( $classes as $class )
										<tr>
											<td>{{ $class -> id }}</td>
											<td><a href="{{ url('class') . '/' . $class -> id }}">{{ $class -> name }}</a></td>
					...
					<div role="tabpanel" class="tab-pane fade" id="cstudents">
						...
						<div class="panel-body">
							<table class="table table-hover">
								<tr><th>所在班级</th><th>英文名</th><th>姓名</th><th>性别</th><th>年龄</th><!--<th>年级</th>--><th>积分</th></tr>
								@foreach ( $classs as $student )
									...
									@else
										<tr>
										@endif
											<td><a href="{{ url('class') . '/' . $student -> classes -> id }}">{{ $student -> classes -> name }}</a></td>
											<td>{{ $student -> ename }}</td>
											<td><a href="{{ url('lessonsinfo') . '/' . $student -> id }}">{{ $student -> name }}</a> &emsp;<span class="{{ $student -> getfuxi() > 5 ? 'color-danger' : ( $student -> getfuxi() == 5 ? 'color-warning' : 'hidden' ) }}">复</span>&emsp;<span class="{{ $student -> getjingpin() > 5 ? 'color-danger' : ( $student -> getjingpin() == 5 ? 'color-warning' : 'hidden' ) }}">精</span></td>
											<td>{{ $student -> sex }}</td>
											<td>{{ Carbon\Carbon::now() -> diffInYears($student -> birthday , 'true') }}岁</td>
